 ...
 * App config
 */
app.config(function ($qProvider, $locationProvider, $compileProvider, $interpolateProvider) {
...
app.controller("MainController", function (API, Song, $timeout, $interval, $scope, $window, $filter) {
  ...
  var getParty = function (newParty) {
     ...
     */
      ...
      API.get("generate", null, null, function (data) {
        ...
        getParty(data.data.party.substr(0, 6).toUpperCase());
      });
      return;
    }
    /**
  ...
  var getSongs = function () {
    ...
    if ($scope.songs.length > 0) {
      payload.last = $scope.songs[$scope.songs.length - 1].id;
    }
  ...
  $scope.addSong = function (form) {
    ...
    API.post("add-song", payload, null, function (data) {
